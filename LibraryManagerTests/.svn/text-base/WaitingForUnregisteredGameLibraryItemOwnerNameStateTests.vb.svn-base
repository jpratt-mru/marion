...
<TestFixture()> _
Public Class WaitingForUnregisteredGameOwnerNameStateTests
    ...
    <Test()> _
    Public Sub ReactToCancelEntry_MovesToWaitingForUnregisteredGameNameState_ContextStaysSame()
    ...
    <Test()> _
    Public Sub ReactToCorrectGameOwnerNameEntry_MovesToWaitingForBorrowerBarcodeState_LibraryHasNewItem()
        borrowingMachineTester.SimulateStringInput(unregisteredGame.Owner)

        Dim actualContext As CurrentBorrowingContext = currentBorrowingContext

        ' NOTE this only works if the clearBorrowingFormAndChangeStateTimer.Start() is commented out of the WaitingForUnregisterdGameOwnerNameState class
        Assert.IsInstanceOf(GetType(WaitingForBorrowerBarcodeState), borrowingMachine.CurrentState)
        Assert.IsTrue(currentBorrowingContext.ContainsGame(unregisteredGame.Barcode))
    End Sub
