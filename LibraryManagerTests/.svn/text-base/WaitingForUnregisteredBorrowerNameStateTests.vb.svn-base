...
<TestFixture()> _
Public Class WaitingForUnregisteredBorrowerNameStateTests
    ...
    Public Sub ReactToUnknownBarcode_StaysInCurrentState_ContextStaysSame()
        ...
        Assert.IsInstanceOf(GetType(WaitingForUnregisteredBorrowerNameState), borrowingMachine.CurrentState)
    ...
    <Test()> _
    Public Sub ReactToCancelEntry_MovesToWaitingForBorrowerBarcodeState_ContextStaysSame()
    ...
    <Test()> _
    Public Sub ReactToFirstAndLastNameEntry_MovesToWaitingForGameBarcodeState_BorrowersHasNewItem()
        borrowingMachineTester.SimulateStringInput(unregisteredBorrower.FirstName & " " & unregisteredBorrower.LastName)

        Dim actualContext As CurrentBorrowingContext = currentBorrowingContext

        ' NOTE this only works if the clearBorrowingFormAndChangeStateTimer.Start() is commented out of the WaitingForUnregisteredBorrowerNameState class
        'Assert.IsInstanceOf(GetType(WaitingForGameBarcodeState), borrowingMachine.CurrentState)
        Assert.AreEqual(unregisteredBorrower, currentBorrowingContext.CurrentBorrower(registeredGameCheckedInInitially.Barcode))
    End Sub
    ...
    <Test()> _
    Public Sub ReactToSingleNameEntry_StaysInCurrentState_ContextStaysSame()
        borrowingMachineTester.SimulateStringInput(unregisteredBorrower.FirstName)
    ...
    <Test()> _
    Public Sub ReactToMoreThanTwoNameEntry_StaysInCurrentState_ContextStaysSame()
        borrowingMachineTester.SimulateStringInput(unregisteredBorrower.FirstName + " foo " + unregisteredBorrower.LastName)
