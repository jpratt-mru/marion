...
<TestFixture()> _
Public Class WaitingForBorrowerBarcodeStateTests
    ...
    <Test()> _
    Public Sub ReactToUnregisteredBorrowerBarcodeEntry_MovesToWaitingForUnregisteredBorrowerNameState_ContextStaysSame()
    ...
    <Test()> _
    Public Sub ReactToBorrowerWithNoGameOutBarcodeEntry_MovesToWaitingForGameBarcodeState_ItemCheckedOutToBorrower()
        borrowingMachineTester.SimulateStringInput(registeredBorrowerWithNoItemOutInitially.Barcode.ToString)

        Dim actualContext As CurrentBorrowingContext = currentBorrowingContext

        ' NOTE this only works if the clearBorrowingFormAndChangeStateTimer.Start() is commented out of the WaitingForBorrowerBarcodeState class
        Assert.IsInstanceOf(GetType(WaitingForGameBarcodeState), borrowingMachine.CurrentState)
        Assert.AreEqual(registeredBorrowerWithNoItemOutInitially, actualContext.CurrentBorrower(registeredGameCheckedInInitially.Barcode))
